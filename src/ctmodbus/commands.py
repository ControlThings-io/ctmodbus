...
ctmodbus.version = open('VERSION').read()
ctmodbus.description = 'a security professional\'s swiss army knife for interacting with Modbus devices'
...
def do_connect_ascii(device:str):
    ...
    """
    assert (ctmodbus.session == None), 'Session already open.  Close first.'  # ToDo assert session type
    ...
    message_dialog(
        title = 'Success',
        text = 'Session opened with {}'.format(valid_device) )
...
def do_close():
    ...
    message_dialog(
        title = 'Success',
        text = 'Session closed with {}'.format(ctmodbus.session) )
    ctmodbus.session = None
    return None
